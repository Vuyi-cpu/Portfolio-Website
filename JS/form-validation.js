function submitForm() {
  ...
  let isValid = true;
  if (!nameValue)    { document.querySelector('#name-err').classList.add('show');  isValid = false; }
  if (!emailValue || !/^[^@]+@[^@]+\.[^@]+$/.test(emailValue)) {
    document.querySelector('#email-err').classList.add('show'); isValid = false;
  }
  if (!messageValue) { document.querySelector('#msg-err').classList.add('show');   isValid = false; }
